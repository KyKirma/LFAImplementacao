...
def leituraDaPalavra(estado_inicial, estados_finais, regras, palavra):
...
def main():
	...
	while palavra != "":
		...
		if automatoValido(alfabeto, estados, estados_finais, regras):
			print("Processamento: ")
			validade = leituraDaPalavra(estado_inicial, estados_finais, regras, palavra)
			if validade == True:
				print("\nPalavra Valida")
				palavra = input("Digite uma palavra: ")
			else:
...
if len(sys.argv) != 2:
	print("Requer um arquivo txt processar")
	print("Tente: python3 Projeto.py <arquivo-txt>")
else:
	print(main())